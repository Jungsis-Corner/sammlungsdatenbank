...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM `Hersteller` WHERE `ID` = $del_id");
    header('Location: manage_hersteller.php?page=' . $page . '&q=' . urlencode($search));
    exit;
}
...
$name = '';
if ($id) {
    $stmt = $conn->prepare("SELECT `Hersteller` FROM `Hersteller` WHERE `ID` = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = trim($_POST['Hersteller'] ?? '');
    if ($id) {
        $stmt = $conn->prepare("UPDATE `Hersteller` SET `Hersteller` = ? WHERE `ID` = ?");
        $stmt->bind_param('si', $name, $id);
    } else {
        $stmt = $conn->prepare("INSERT INTO `Hersteller` (`Hersteller`) VALUES (?)");
        $stmt->bind_param('s', $name);
    }
    $stmt->execute();
    header('Location: manage_hersteller.php?q=' . urlencode($search));
    exit;
}

/* ---------- Zählung & Liste ---------- */
$where = ($search !== '')
    ? "WHERE `Hersteller` LIKE '%" . $conn->real_escape_string($search) . "%'"
    : '';

$total = 0;
if ($r = $conn->query("SELECT COUNT(*) FROM `Hersteller` $where")->fetch_row()) {
    $total = (int)$r[0];
}
$pages = max(1, (int)ceil($total / $limit));

$sql = "SELECT `ID`,`Hersteller`
        FROM `Hersteller`
        $where
        ORDER BY `Hersteller`
...
<html lang="de">
<head>
  <?php $page_title = 'Hersteller verwalten'; include __DIR__ . '/inc_head_admin.php'; ?>
</head>
  ...
  <main class="page"><!-- zentriert & breite begrenzt -->
    ...
    <div class="top-bar">
      ...
      <form method="get" class="top-bar-search">
        <input type="text" name="q" placeholder="Hersteller suchen…" value="<?= htmlspecialchars($search) ?>">
        <button type="submit" class="btn is-small">🔍</button>
        <?php if ($search !== ''): ?>
          <button type="button" class="btn btn-red is-small" onclick="location.href='manage_hersteller.php'">✖</button>
        <?php endif; ?>
      </form>
    </div>
...
<div class="form-line-center">
  <form method="post" class="admin-inline-form">
    <input type="text" name="Hersteller" placeholder="Name des Herstellers" value="<?= htmlspecialchars($name) ?>">
        ...
        <tbody>
          <?php while ($row = $list->fetch_assoc()): ?>
            <tr>
              ...
              <td class="actions">
                <a class="btn is-small" href="manage_hersteller.php?id=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>" title="Bearbeiten">✏️</a>
                <a class="btn is-small btn-red"
                   href="manage_hersteller.php?delete=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>"
