...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM `Standort` WHERE `ID` = $del_id");
    header('Location: manage_standort.php?page=' . $page . '&q=' . urlencode($search));
    exit;
}
...
$name = '';
if ($id) {
    $stmt = $conn->prepare("SELECT `Standort` FROM `Standort` WHERE `ID` = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = trim($_POST['Standort'] ?? '');
    if ($id) {
        $stmt = $conn->prepare("UPDATE `Standort` SET `Standort` = ? WHERE `ID` = ?");
        $stmt->bind_param('si', $name, $id);
    } else {
        $stmt = $conn->prepare("INSERT INTO `Standort` (`Standort`) VALUES (?)");
        $stmt->bind_param('s', $name);
    }
    $stmt->execute();
    header('Location: manage_standort.php?q=' . urlencode($search));
    exit;
}

/* ---------- Zählung & Liste ---------- */
$where = ($search !== '')
    ? "WHERE `Standort` LIKE '%" . $conn->real_escape_string($search) . "%'"
    : '';

$total = 0;
if ($r = $conn->query("SELECT COUNT(*) FROM `Standort` $where")->fetch_row()) {
    $total = (int)$r[0];
}
$pages = max(1, (int)ceil($total / $limit));

$sql = "SELECT `ID`,`Standort`
        FROM `Standort`
        $where
        ORDER BY `Standort`
...
<html lang="de">
<head>
  <?php $page_title = 'Standort verwalten'; include __DIR__ . '/inc_head_admin.php'; ?>
</head>
  ...
  <main class="page"><!-- zentriert & breite begrenzt -->
    ...
    <div class="top-bar">
      ...
      <form method="get" class="top-bar-search">
        <input type="text" name="q" placeholder="Standort suchen…" value="<?= htmlspecialchars($search) ?>">
        <button type="submit" class="btn is-small">🔍</button>
        <?php if ($search !== ''): ?>
          <button type="button" class="btn btn-red is-small" onclick="location.href='manage_standort.php'">✖</button>
        <?php endif; ?>
      </form>
    </div>
...
<div class="form-line-center">
  <form method="post" class="admin-inline-form">
    <input type="text" name="Standort" placeholder="Name des Standorts" value="<?= htmlspecialchars($name) ?>">
        ...
        <tbody>
          <?php while ($row = $list->fetch_assoc()): ?>
            <tr>
              ...
              <td class="actions">
                <a class="btn is-small" href="manage_standort.php?id=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>" title="Bearbeiten">✏️</a>
                <a class="btn is-small btn-red"
                   href="manage_standort.php?delete=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>"
