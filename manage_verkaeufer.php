...
if (isset($_GET['delete'])) {
    ...
    // ACHTUNG: Tabelle + Spalte mit Umlaut und Großbuchstaben
    $conn->query("DELETE FROM `Verkäufer` WHERE `ID` = $del_id");
    header('Location: manage_verkaeufer.php?page=' . $page . '&q=' . urlencode($search));
    exit;
}
...
$name = '';
if ($id) {
    $stmt = $conn->prepare("SELECT `Verkäufer` FROM `Verkäufer` WHERE `ID` = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Formularfeld ohne Umlaut -> DB-Spalte MIT Umlaut
    $name = trim($_POST['verkaeufer'] ?? '');
    if ($id) {
        $stmt = $conn->prepare("UPDATE `Verkäufer` SET `Verkäufer` = ? WHERE `ID` = ?");
        $stmt->bind_param('si', $name, $id);
    } else {
        $stmt = $conn->prepare("INSERT INTO `Verkäufer` (`Verkäufer`) VALUES (?)");
        $stmt->bind_param('s', $name);
    }
    $stmt->execute();
    header('Location: manage_verkaeufer.php?q=' . urlencode($search));
    exit;
}

/* ---------- Zählung & Liste ---------- */
$where = ($search !== '')
    ? "WHERE `Verkäufer` LIKE '%" . $conn->real_escape_string($search) . "%'"
    : '';

$total = 0;
if ($r = $conn->query("SELECT COUNT(*) FROM `Verkäufer` $where")->fetch_row()) {
    $total = (int)$r[0];
}
$pages = max(1, (int)ceil($total / $limit));

$sql = "SELECT `ID`,`Verkäufer`
        FROM `Verkäufer`
        $where
        ORDER BY `Verkäufer`
...
<html lang="de">
<head>
  <?php $page_title = 'Verkäufer verwalten'; include __DIR__ . '/inc_head_admin.php'; ?>
</head>
<body class="admin-page">
  <main class="page">
    <header class="admin-header">
      <h1>Verkäufer verwalten</h1>
    ...
    <div class="top-bar">
      ...
      <form method="get" class="top-bar-search">
        <input type="text" name="q" placeholder="Verkäufer suchen…" value="<?= htmlspecialchars($search) ?>">
        <button type="submit" class="btn is-small">🔍</button>
        <?php if ($search !== ''): ?>
          <button type="button" class="btn btn-red is-small" onclick="location.href='manage_verkaeufer.php'">✖</button>
        <?php endif; ?>
      </form>
    </div>
    ...
    <div class="form-line-center">
      <form method="post" class="admin-inline-form">
        <input type="text" name="verkaeufer" placeholder="Name des Verkäufers" value="<?= htmlspecialchars($name) ?>">
        ...
        <tbody>
          <?php while ($row = $list->fetch_assoc()): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($row['Verkäufer']) ?></td>
              <td class="actions">
                <a class="btn is-small" href="manage_verkaeufer.php?id=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>" title="Bearbeiten">✏️</a>
                <a class="btn is-small btn-red"
                   href="manage_verkaeufer.php?delete=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>"
