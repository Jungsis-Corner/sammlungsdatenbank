...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM `Material` WHERE `ID` = $del_id");
    header('Location: manage_material.php?page=' . $page . '&q=' . urlencode($search));
    exit;
}
...
$name = '';
if ($id) {
    $stmt = $conn->prepare("SELECT `Material` FROM `Material` WHERE `ID` = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $stmt = $conn->prepare("UPDATE `Material` SET `Material` = ? WHERE `ID` = ?");
    ...
    $stmt->execute();
    header('Location: manage_material.php?q=' . urlencode($search));
    exit;
}

/* ---------- Zählung & Liste ---------- */
$where = ($search !== '')
    ? "WHERE `Material` LIKE '%" . $conn->real_escape_string($search) . "%'"
    : '';

$total = 0;
if ($r = $conn->query("SELECT COUNT(*) FROM `Material` $where")->fetch_row()) {
    $total = (int)$r[0];
}
...
<html lang="de">
<head>
  <?php $page_title = 'Material verwalten'; include __DIR__ . '/inc_head_admin.php'; ?>
</head>
  ...
  <main class="page"><!-- zentriert & breite begrenzt -->
    ...
    <div class="top-bar">
      ...
      <form method="get" class="top-bar-search">
        <input type="text" name="q" placeholder="Material suchen…" value="<?= htmlspecialchars($search) ?>">
        <button type="submit" class="btn is-small">🔍</button>
        <?php if ($search !== ''): ?>
          <button type="button" class="btn btn-red is-small" onclick="location.href='manage_material.php'">✖</button>
        <?php endif; ?>
      </form>
    </div>
...
<div class="form-line-center">
  <form method="post" class="admin-inline-form">
    <input type="text" name="Material" placeholder="Name des Materials" value="<?= htmlspecialchars($name) ?>">
        ...
        <tbody>
          <?php while ($row = $list->fetch_assoc()): ?>
            <tr>
              ...
              <td class="actions">
                <a class="btn is-small" href="manage_material.php?id=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>" title="Bearbeiten">✏️</a>
                <a class="btn is-small btn-red"
                   href="manage_material.php?delete=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>"
