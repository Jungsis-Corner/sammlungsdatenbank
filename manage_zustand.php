...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM `Zustand` WHERE `ID` = $del_id");
    header('Location: manage_zustand.php?page=' . $page . '&q=' . urlencode($search));
    exit;
}
...
$name = '';
if ($id) {
    $stmt = $conn->prepare("SELECT `Zustand` FROM `Zustand` WHERE `ID` = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = trim($_POST['Zustand'] ?? '');
    if ($id) {
        $stmt = $conn->prepare("UPDATE `Zustand` SET `Zustand` = ? WHERE `ID` = ?");
        $stmt->bind_param('si', $name, $id);
    } else {
        $stmt = $conn->prepare("INSERT INTO `Zustand` (`Zustand`) VALUES (?)");
        $stmt->bind_param('s', $name);
    }
    $stmt->execute();
    header('Location: manage_zustand.php?q=' . urlencode($search));
    exit;
}

/* ---------- Zählung & Liste ---------- */
$where = ($search !== '')
    ? "WHERE `Zustand` LIKE '%" . $conn->real_escape_string($search) . "%'"
    : '';

$total = 0;
if ($r = $conn->query("SELECT COUNT(*) FROM `Zustand` $where")->fetch_row()) {
    $total = (int)$r[0];
}
$pages = max(1, (int)ceil($total / $limit));

$sql = "SELECT `ID`,`Zustand`
        FROM `Zustand`
        $where
        ORDER BY `Zustand`
...
<html lang="de">
<head>
  <?php $page_title = 'Zustand verwalten'; include __DIR__ . '/inc_head_admin.php'; ?>
</head>
  ...
  <main class="page"><!-- zentriert & breite begrenzt -->
    ...
    <div class="top-bar">
      ...
      <form method="get" class="top-bar-search">
        <input type="text" name="q" placeholder="Zustand suchen…" value="<?= htmlspecialchars($search) ?>">
        <button type="submit" class="btn is-small">🔍</button>
        <?php if ($search !== ''): ?>
          <button type="button" class="btn btn-red is-small" onclick="location.href='manage_zustand.php'">✖</button>
        <?php endif; ?>
      </form>
    </div>
...
<div class="form-line-center">
  <form method="post" class="admin-inline-form">
    <input type="text" name="Zustand" placeholder="Name des Zustands" value="<?= htmlspecialchars($name) ?>">
        ...
        <tbody>
          <?php while ($row = $list->fetch_assoc()): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($row['Zustand']) ?></td>
              <td class="actions">
                <a class="btn is-small" href="manage_zustand.php?id=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>" title="Bearbeiten">✏️</a>
                <a class="btn is-small btn-red"
                   href="manage_zustand.php?delete=<?= (int)$row['ID'] ?>&q=<?= urlencode($search) ?>"
